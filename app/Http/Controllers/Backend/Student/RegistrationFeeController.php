...
class RegistrationFeeController extends Controller
{
    ...
    public function RegFeeClassData(Request $request){
         ...
         // $data['allStudent'] = AssignStudent::with(['discount','student'])->where($where)->get();

         $allStudent = AssignStudent::with(['discount'])->where($where)->where($where)->get();
         // $data['registrationfee'] = FeeCategoryAmount::where('fee_category_id','1')->where('class_id',$request->class_id)->first();
         // dd($data['registrationfee']);
    ...
    public function RegFeePayslip(Request $request)
    {
        ...
        $allStudent['details'] = AssignStudent::with([])->where('student_id',$student_id)->where('class_id',$class_id)->first();
